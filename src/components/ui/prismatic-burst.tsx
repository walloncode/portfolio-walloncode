...
// TypeScript + Tailwind port of React Bits' PrismaticBurst (JS + CSS variant).
// The original ships a `.css` file with the container rules; here they live as
// Tailwind classes on the container instead, so no extra stylesheet is needed.
...
export type PrismaticBurstAnimation = "rotate" | "rotate3d" | "hover";

export interface PrismaticBurstProps {
  intensity?: number;
  speed?: number;
  animationType?: PrismaticBurstAnimation;
  colors?: string[];
  distort?: number;
  paused?: boolean;
...
export default function PrismaticBurst({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const tex = gradTexRef.current?.texture;
      if (glCtx && tex) {
        try {
  ...
  useEffect(() => {
    ...
    const animTypeMap: Record<PrismaticBurstAnimation, number> = {
      rotate: 0,
      rotate3d: 1,
      hover: 2,
    };
